...
as
begin
 select IdAlumno,Codigo,Nombres,Apellidos,DocumentoIdentidad,Activo from ALUMNO
end
...
begin
    ...
    BEGIN
        INSERT INTO ALUMNO (ValorCodigo, Codigo, Nombres, Apellidos, DocumentoIdentidad)
        VALUES (
...
as
begin
 select IdDocente,Codigo,DocumentoIdentidad,Nombres,Apellidos,Email,NumeroTelefono,Activo from DOCENTE
 order by Nombres
end
...
begin try
	...
	BEGIN TRANSACTION
		...
		insert into @alumno(idalumno,documentoidentidad,nombres,apellidos)
		select 
		...
		insert into @apoderado(tiporelacion,documentoidentidad,nombres,apellidos)
		select 
		...
		--REGISTRAMOS DOCENTE
		insert into APODERADO(TipoRelacion,Nombres,Apellidos,DocumentoIdentidad)
		select 
		tiporelacion,
		nombres,
		apellidos,
...
begin
  ...
  convert(char(10),
  n.DescripcionTurno[Turno],gs.DescripcionGrado[Grado],gs.DescripcionSeccion[Seccion]
  from MATRICULA m
  inner join ALUMNO a on a.IdAlumno = m.IdAlumno
